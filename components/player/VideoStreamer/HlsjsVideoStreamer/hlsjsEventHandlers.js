...
const getHlsjsEventHandlers = ({
  ...
  function handleActualError(detail) {
    ...
    if (hls && playbackError.severity === 'FATAL') {
      ...
      if (autoRecoverStreamErrors && detail.type === _hls.default.ErrorTypes.NETWORK_ERROR) {
        hls.startLoad();
      } else if (autoRecoverStreamErrors && detail.type === _hls.default.ErrorTypes.MEDIA_ERROR) {
