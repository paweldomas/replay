...
function hlsjsSetup(videoElement, normalizedSource, configuration) {
    ...
    if (_hls.default.isSupported()) {
      ...
      if (licenseUrl) {
        ...
        if (drmDetails && drmDetails.robustness && drmDetails.robustness['com.widevine.alpha']) {
          const _drmDetails$robustnes = drmDetails.robustness['com.widevine.alpha'],
                audio = _drmDetails$robustnes.audio,
                video = _drmDetails$robustnes.video;
