...
const getSourceChangeHandler = (shakaLib, shakaPlayer) => (nextProps, prevProps) => {
  ...
  if (source) {
    ...
    const _shakaConf$requestFil = shakaConf.requestFilter,
          requestFilter = _shakaConf$requestFil === void 0 ? nextProps.shakaRequestFilter : _shakaConf$requestFil,
          _shakaConf$responseFi = shakaConf.responseFilter,
          responseFilter = _shakaConf$responseFi === void 0 ? nextProps.shakaResponseFilter : _shakaConf$responseFi;
    return prepareFilters(shakaPlayer, requestFilter, responseFilter).then(() => prepareDrm(shakaLib, shakaPlayer, source, nextProps.configuration)).then(() => shakaPlayer.load(source.streamUrl, source.startPosition)).catch(err => {
